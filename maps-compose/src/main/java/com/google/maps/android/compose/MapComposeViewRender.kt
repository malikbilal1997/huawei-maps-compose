...
/**
 * A unique tag used to identify the internal `NoDrawContainerView` that we attach to the MapView.
 * This replaces the need for an XML-defined ID and ensures that the same hidden container can be
 * retrieved on subsequent calls instead of creating multiple instances.
 */
private const val NO_DRAW_CONTAINER_VIEW_TAG = "no_draw_container_view_tag"

/**
 * Prepares [view] for a single render by temporarily attaching it as a descendant of this [MapView]
...
private fun MapView.ensureContainerView(): NoDrawContainerView {
  return findViewWithTag(NO_DRAW_CONTAINER_VIEW_TAG)
    ?: NoDrawContainerView(context)
      .apply {
        id = View.generateViewId()
        tag = NO_DRAW_CONTAINER_VIEW_TAG
      }
      .also(::addView)
}
